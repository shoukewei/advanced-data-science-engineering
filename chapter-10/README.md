# dskit - A Reusable Data Science Framework

`dskit` is an installable Python package for reproducible, configuration-driven
...
The package is still lightweight enough to use as a toolkit, but it now also has
a small framework layer: you can register custom pipeline steps, declare them in
configuration, and let `dskit` call them during execution.
...
```bash
pip install "dskit[polars]"
pip install "dskit[yaml]"
```
