...
def test_verify_no_leakage_passes_and_fails():
    ...
    # should pass
    verify_no_leakage(X_train, X_test, scaler_params)

    # should fail
    X_train2 = pd.DataFrame({"a": [1.0, 1.0, 1.0]})

    with pytest.raises(AssertionError):
        verify_no_leakage(X_train2, X_test, scaler_params)